    ...
    public partial class Converter
    {
        ...
        private Field ExtractColumnProperties(SQLiteConnection conn, string TableName, string input, ref string MarkPrimaryKeyColumnName)
        {
            ...
            if (m.Success)
            {
                ...
                length = Shove._Convert.StrToInt(m.Groups["L1"].Value, 0);
            }
            ...
            if ((t_type == "VARCHAR") || (t_type == "NVARCHAR"))
            {
                ...
                dr.Read();
                int max_length = Shove._Convert.StrToInt(dr[0].ToString(), 0) * 2;
                dr.Close();
            ...
            if (!String.IsNullOrEmpty(DefaultValue))
            {
                ...
                {
                    DefaultValue = Shove._Convert.StrToInt(DefaultValue, 0).ToString();
                }
                else if ((t_type == "FLOAT") || (t_type == "REAL") || (t_type == "NUMERIC"))
                {
                    DefaultValue = Shove._Convert.StrToDouble(DefaultValue, 0).ToString();
                }
            }
        ...
        private string FilterViewStatement(string input)
        {
            ...
            {
                input = Shove._String.ReplaceAt(input, '[', input.IndexOf('\"'));
                input = Shove._String.ReplaceAt(input, ']', input.IndexOf('\"'));
            }
