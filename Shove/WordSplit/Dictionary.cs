...
namespace Shove.WordSplit
{
    ...
    public class Dictionary
    {
        ...
        //				10 - 汉字数字， 11 - 汉字量词， 12 - 汉字连字， 13 - 普通汉字
        {
            ...
            int Unicode = (int)ch;
            if (((Unicode >= (int)'a') && (Unicode <= (int)'z')) ||
                ((Unicode >= (int)'A') && (Unicode <= (int)'Z')) ||
                ((Unicode >= (int)'Ａ') && (Unicode <= (int)'Ｚ')) ||
                ((Unicode >= (int)'ａ') && (Unicode <= (int)'ｚ')) ||
                ("0123456789０１２３４５６７８９".IndexOf(sWord) >= 0))
                return 0;

            if ("!@#$%^&*()_-+=|\\~`[]{}<>./［］｛｝！·＃￥％…—＊（）".IndexOf(sWord) >= 0)
                return 1;

            if ((Unicode >= 19968) && (Unicode <= 40868))
            {
                if ("第一二两三四五六七八九十壹贰叁肆伍陆柒捌玖拾零百千万亿兆佰仟卅几整".IndexOf(sWord) >= 0)
                    return 10;
                if ("元角分毛点时半刻秒岁个把盒包箱条件张页本块根套面幅付副吨斤克两钱磅米厘听毫丈尺寸里卷打瓶罐桶只支袋部次台辆年月日天号周折头份成位名种楼路层项级封度微升片粒双匹票盅碟碗盏方滴册箩坨杯口滩笼扇筐簸串吊挂捆团担家壶轮令栋发株沓窝群枚具棵枝管道颗款朵缕堂盘节贴剂服座幢堵间处所架艘趟爿手桩宗笔通场记喷则首篇门尊股席倍餐回顿句泓挺惯身叠锅束地绺出集段曲波员池声刀帘代边组届类批步等章环列期胎维品更局户季帮遍簇丛端堆队对番格伙栏篮例流抹排枪区圈拳提招桌着大".IndexOf(sWord) >= 0)
                    return 11;
                if ("和及与跟同或最已而等用把上来".IndexOf(sWord) >= 0)
                    return 12;
                return 13;
            }
            return -1;
