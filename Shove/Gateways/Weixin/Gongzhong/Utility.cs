...
namespace Shove.Gateways.Weixin.Gongzhong
{
    ...
    public class Utility
    {
        ...
        public static bool DownloadTwo_Dimension_Code(string ticket, ref string saveFilePath, ref string errorDescription)
        {
            //系统时间做为二维码图片名称
            ...
            try
            {
                ...
                using (Stream stream = request.GetResponse().GetResponseStream())
                {
                    ...
                    using (FileStream fs = File.Create(HttpContext.Current.Server.MapPath(saveFilePath) + dataTime + ".jpg"))
                    {
