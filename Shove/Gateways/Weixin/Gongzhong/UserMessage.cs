...
namespace Shove.Gateways.Weixin.Gongzhong
{
    ...
    public class UserMessage
    {
        ...
        public static bool Handle(string toKen, DelegateGetData GetData, ref string errorDescription)
        {
            errorDescription = "";
            #region 验证请求的信息是否来自微信服务器

            bool sucessed = ValidSignature(toKen, ref errorDescription);
            ...
            try
            {
                ...
                if (MsgType.ToLower().Equals("event")) //用户点击菜单
                {
                    ...
                    else if (Event.ToUpper().Equals("MASSSENDJOBFINISH"))//群发失败信息返回
                    {
                        ...
                        long _SentCount = Shove._Convert.StrToLong(xn.SelectSingleNode("//SentCount").InnerText, -1);
                        long _ErrorCount = Shove._Convert.StrToLong(xn.SelectSingleNode("//ErrorCount").InnerText, -1);
            ...
            for (int i = 0; i < replyMessages.Count; i++)
            {
                if (replyMessages[i] is TextMessage)
                {
                    ...
                    int len = Shove._String.GetLength(message.Content);
                ...
                else if (replyMessages[i] is ImageTextMessage)
                {
                    ...
                    if (Convert.ToInt32(message.ArticleCount) > 10 || replyMessages.Count > 10)
                    {
            ...
            HttpContext.Current.Response.Write(reply.ToString());
        ...
        private static bool ValidSignature(string toKen, ref string errorDescription)
        {
            errorDescription = "";
