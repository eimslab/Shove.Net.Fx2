...
namespace Shove.Gateways
{
    ...
    public class GeneralRestGateway
    {
        ...
        /// <returns></returns>
        public static int Handle(string key, int allowTimespanSeconds, DelegateHandleRequest HandleRequest, ref string errorDescription)
        {
            errorDescription = "";
            ...
            string[] keys = req.QueryString.AllKeys;
            ...
            for (int i = 0; i < req.QueryString.Count; i++)
            {
                if (keys[i] == "_s")
                {
                    _s = req.QueryString["_s"];

                    continue;
                }
                else if (keys[i] == "_t")
                {
                    _t = HttpUtility.UrlDecode(req.QueryString["_t"], Encoding.UTF8);
                }

                parameters.Add(keys[i], HttpUtility.UrlDecode(req.QueryString[keys[i]], Encoding.UTF8));
            }
            ...
            DateTime timestamp = _Convert.StrToDateTime(_t, DateTime.Now.AddYears(-30).ToString("yyyy-MM-dd HH:mm:ss"));
            ...
            if (String.Compare(_Security.Encrypt.MD5(signData + key, Encoding.UTF8), _s, true) != 0)
            {
        ...
        public static string BuildUrl(string urlBase, string key, Dictionary<string, object> parameters)
        {
            ...
            urlBase += "&_s=" + _Security.Encrypt.MD5(signData + key, Encoding.UTF8);
