...
namespace Shove.HTML.SgmlReader
{
	...
	public class Entity 
	{
		...
		public string ScanToken(StringBuilder sb, string term, bool nmtoken) 
		{
			...
			{
				throw new Exception(
					System.String.Format("Invalid name start character '{0}'", ch));
			}
			while (ch != Entity.EOF && term.IndexOf(ch)<0) 
			{
				...
				{
					throw new Exception(
						System.String.Format("Invalid name character '{0}'", ch));
				}
				ch = ReadChar();
			}
			return sb.ToString();
		...
		public string ExpandCharEntity() 
		{
			...
			if (this.Html && v >= 0x80 & v <= 0x9F) 
			{
				...
				return System.Convert.ToChar(unicode).ToString();
			}
			return System.Convert.ToChar(v).ToString();
		}
		...
		public void Error(string msg, char ch) 
		{
			...
			throw new Exception(System.String.Format(msg, str));
		}
		...
		{
			throw new Exception(System.String.Format(msg, x));
		}
		...
		{
			throw new Exception(System.String.Format(msg, arg));
		}
	...
	internal class HtmlStream : TextReader 
	{
		...
		internal string SniffName()
		{
			...
			char ch = (char)c;
			int start = pos;
			while (pos < used && ( Char.IsLetterOrDigit(ch) || ch == '-' || ch == '_' || ch == ':')) 
			{
				ch = buffer[++pos];
			}
	...
	public class ContentModel
	{
		...
		public void SetDeclaredContent(string dc)
		{
			switch (dc) 
			{
				...
				default:
					throw new Exception(
						System.String.Format("Declared content type '{0}' is not supported", dc));
			}
		}
	...
	public class Group
	{
		...
		public void AddConnector(char c)
		{
			...
			{
				throw new Exception(
					System.String.Format("Missing token before connector '{0}'.", c)
					);
			}
			...
			{
				throw new Exception(
					System.String.Format("Connector '{0}' is inconsistent with {1} group.", c, GroupType.ToString())
					);
			}
			GroupType = gt;
		}
	...
	public class AttDef
	{
		...
		public bool SetPresence (string token)
		{
			...
			{
				throw new Exception(System.String.Format("Attribute value '{0}' not supported", token));
			}
			return hasDefault;
		}
	}
