...
// 
// $Log: Logger.cs,v $
// Revision 1.35  2012/11/30 01:28:29  bruceb
// SetTag changes
//
// Revision 1.34  2012-02-09 03:29:28  hans
// Fix NRE in FormattedText (when text=null).
//
...
namespace EnterpriseDT.Util.Debug
{
	...
	public class Logger
	{
        ...
        {
            // commented out because it causes problems when method that call SetTag are nested
            //lock (threadTags)
            //{
            //    threadTags.Remove(Thread.CurrentThread.ManagedThreadId);
            //}
        }
        ...
        private static string GetTag()
        {
            lock (threadTags)
            {
                if (threadTags.ContainsKey(Thread.CurrentThread.ManagedThreadId))
                    return threadTags[Thread.CurrentThread.ManagedThreadId].GetLogTag();
                else
                    return "";
            }
        }
        ...
        private static Dictionary<int, ILogTag> threadTags = new Dictionary<int, ILogTag>();
