...
// 获取所有 Part
function ShoveWebUI_ShovePart2_GetParts(isWithLocked, isWithHidden) {
    var objParts = [];
    ...
    for (var i = 0; i < objDivs.length; i++) {
        ...
        if (ShoveWebUITypeName != "ShovePart2") {
            continue;
        }
...
function ShoveWebUI_ShovePart2_GetParentLeft(sender) {
    if(sender==null)
    {
        return ;
    }
...
function ShoveWebUI_ShovePart2_GetParentTop(sender) {
    var o_ParentDiv = sender.parentNode;
    var ParentTop = "0px";
...
function ShoveWebUI_ShovePart2_GetParentWidth(sender) {
    var o_ParentDiv = sender.parentNode;
    var ParentWidth = "0px";
...
function ShoveWebUI_ShovePart2_OffsetLeft(sender, ParentLeft) {
    ...
    if (!ParentLeft) {
        ParentLeft = ShoveWebUI_ShovePart2_GetParentLeft(sender);
    }
...
if (window.addEventListener) {
    window.addEventListener("resize", ShoveWebUI_ShovePart2_OnResize, false);
}
else {
    window.onresize = function ShoveWebUI_ShovePart2_OnResize_IE() {
        if (document.readyState == "complete") {
            ShoveWebUI_ShovePart2_OnResize();
        }
    }
}

function ShoveWebUI_ShovePart2_OnResize(objParts) {
    if (!objParts) {
        objParts = ShoveWebUI_ShovePart2_GetParts(true, false);
    }
    ...
    var ParentLeft = ShoveWebUI_ShovePart2_GetParentLeft(objParts[0]);

    for (var i = 0; i < objParts.length; i++) {
        ShoveWebUI_ShovePart2_OffsetLeft(objParts[i], ParentLeft);
    }
}

if (document.addEventListener) {
    document.addEventListener("DOMContentLoaded", ShoveWebUI_ShovePart2_OffsetTop, false);
}
else {
    document.onreadystatechange = function ShoveWebUI_ShovePart2_OffsetTop_IE() {
        if (document.readyState == "complete") {
            ShoveWebUI_ShovePart2_OffsetTop();
        }
    }
}

function ShoveWebUI_ShovePart2_OffsetTop() {
    var objParts = ShoveWebUI_ShovePart2_GetParts(true, false).sort(Sort);

    // 调用偏移 Left。
    ShoveWebUI_ShovePart2_OnResize(objParts);
